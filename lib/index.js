...
* @param {Number} [opts.a=0] - minimum value
* @param {Number} [opts.b=1] - maximum value
...
function pdf( x, options ) {
	...
	if ( opts.b <= opts.a ) {
		throw new TypeError( 'pdf()::invalid input options. Parameter `a` must be smaller than `b`.' );
	}


	if ( isNumber( x ) ) {
		return pdf1( x, opts.a, opts.b );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.a, opts.b );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.a, opts.b );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.a, opts.b, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.a, opts.b, opts.accessor );
		}
		return pdf2( out, x, opts.a, opts.b );
	}
	return NaN;
} // end FUNCTION pdf()
